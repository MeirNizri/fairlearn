...
def run_comparisons(moment, metric_fn):
    ...
    for x, y in datasets:
        ...
        # sensitive feature
        sex_ = x['sex']
        sex = x['sex'].apply(lambda k: 0 if k == "Male" else 1)
        ...
        mf_unmitigated = MetricFrame(metrics=metric_fn, y_true=y,
                                     y_pred=y_pred, sensitive_features=sex_)

        logistic_regression_model = MyLogisticRegression(x_dummy.shape[1], 1)
        ...
        mf_mitigated = MetricFrame(metrics=metric_fn, y_true=y, y_pred=y_pred, sensitive_features=sex_)

        # print(mf_unmitigated.difference(method='to_overall'))
